...
{ Original: jcmaster.c ; Copyright (C) 1991-1996, Thomas G. Lane. }
...
{$ifdef C_PROGRESSIVE_SUPPORTED}
    ...
    last_bitpos_ptr := @(last_bitpos[0]);
    for ci := 0 to pred(cinfo^.num_components) do
      for coefi := 0 to pred(DCTSIZE2) do
	last_bitpos_ptr^[coefi] := -1;
{$else}
...
{$ifdef C_PROGRESSIVE_SUPPORTED}
      if (Ss < 0) or (Ss >= DCTSIZE2) or (Se < Ss) or (Se >= DCTSIZE2)
       or (Ah < 0) or (Ah > 13) or (Al < 0) or (Al > 13) then
...
var
{$ifdef C_MULTISCAN_FILES_SUPPORTED} { Meister }
...
{$ifdef C_MULTISCAN_FILES_SUPPORTED}
  ...
  begin
    ...
    comp_infos := jpeg_component_info_list_ptr(cinfo^.comp_info);
