...
begin
  ...
  downsample := my_downsample_ptr(cinfo^.mem^.alloc_small(j_common_ptr(cinfo),
                JPOOL_IMAGE, SIZEOF(my_downsampler)));
  cinfo^.downsample := jpeg_downsampler_ptr (downsample);
  downsample^.pub.start_pass := start_pass_downsample;
  downsample^.pub.downsample := sep_downsample;
  downsample^.pub.need_context_rows := FALSE;
  if (cinfo^.CCIR601_sampling)
  then ERREXIT(j_common_ptr(cinfo), JERR_CCIR601_NOTIMPL);
  {------------- Verify we can handle the sampling factors, and set up }
  {              method pointers                                       }
  compptr := cinfo^.comp_info;
  for ci := 0 to pred(cinfo^.num_components)
  do begin
     if (compptr^.h_samp_factor = cinfo^.max_h_samp_factor)
     and (compptr^.v_samp_factor = cinfo^.max_v_samp_factor)
     then begin
     ...
     and (compptr^.v_samp_factor * 2 = cinfo^.max_v_samp_factor)
     then begin
     ...
     else if ((cinfo^.max_h_samp_factor mod compptr^.h_samp_factor) = 0)
     and ((cinfo^.max_v_samp_factor mod compptr^.v_samp_factor) = 0)
     then begin
     ...
     else ERREXIT(j_common_ptr(cinfo), JERR_FRACT_SAMPLE_NOTIMPL);
     Inc(compptr);
  end;

  {$ifdef INPUT_SMOOTHING_SUPPORTED}
  if (cinfo^.smoothing_factor <> 0) and (not smoothok)
  then TRACEMS(j_common_ptr(cinfo), 0, JTRC_SMOOTH_NOTIMPL);
  {$endif}

end {procedure jinit_downsampler};
